<template>
  <div>
    <!-- Week Days -->
    <div class="grid grid-cols-7 mt-4 text-xs text-center text-gray-500">
    ...
    <div class="grid grid-cols-7 mt-2 text-sm">
      <div
        ...
        :key="day.date"
        :class="[{ 'border-t border-gray-200': dayIdx >= 7 }, 'py-2 relative']"
      >
        <button
...
function hasEventOnDate(dateString) {
  return props.meetings.some((meeting) =>
    isSameDay(parseISO(meeting.startDatetime), parseISO(dateString)),
  );
}

// Function to Get Event Color for a Date
function getEventColor(dateString) {
  const event = props.meetings.find((meeting) =>
    isSameDay(parseISO(meeting.startDatetime), parseISO(dateString)),
  );
